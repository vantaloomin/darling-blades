<!-- source-of-truth: package.json, src/config/rules.ts, src/data/cards/*.ts, src/data/starterDecks.ts, src/data/opponents.ts, src/scenes/, docs/design-system.md, docs/plan-design-system-alignment.md, docs/rules.md, docs/ai.md, docs/art-pipeline.md, docs/roadmap.md, docs/mobile-lan-plan.md, tests/ · last-verified: 2026-07-26
...
The cast is what's different. The **638 collectible cards across six sets** share characters from the officers of **Wei**, **Wu**, **Shu**, and **Jin**, the **Greek pantheon**, tribal **Beastkin**, Ragnarök's graveyard faction, Celtic Fae courts, Arthurian knights, Gothic Monsters, and Dark Tales storybook figures. Every card carries finished cel-shaded gacha-anime art; nothing in the shipped game is programmer-art or a placeholder.

You play or skip a short optional tutorial, claim a free starter deck, crack booster packs to build out your collection, and assemble a 60-card deck in the deck builder. Then you duel: Practice matches against any tower boss or a plain difficulty, the 18-rung **Avatar Gauntlet** (a ladder of named boss opponents, each running a themed deck, reshuffled daily), or a seat at the **Draft** table to build a fresh 40-card deck from passed packs against seven AI rivals.

## Features

- **A 638-card collectible pool across six sets.** Core Set, Ragnarök, Celtic Fae, Arthurian Court, Gothic Monsters, and Dark Tales cover all five WUBRG colors and the same five rarity tiers. Dark Tales, The Cursed Storybook adds 120 cards plus four tokens and has its own set-scoped booster.
...
- **The Avatar Gauntlet**: an 18-rung, 18-floor ladder of named boss opponents (Meng Huo, Hestia, Lupa the Wolfqueen, Hera, Zhurong, Sima Yi, Yohime the Kitsune Matriarch, Cao Cao, the Ragnarök bosses Hel and Brunhild, the Celtic Fae pair of The Morrigan and Titania, the Arthurian summit of Morgan of the Thorn Crown and Artoria, Once and Future Queen, the Gothic Monsters pair of Carmilla and The Bride, and the Dark Tales pair of Glass-Coffin Queen and Abyssal Songstress), each piloting a themed deck and personality. The roster reshuffles every day from a date seed, and the floor you reach sets the AI's strength on a six-tier ladder while the avatar brings its own deck and personality. Gold pays out per rung cleared plus a bonus for a full run. Practice mode lets you challenge any of the 18 bosses directly, or a plain difficulty, with no ladder attached.
...
| Mode | What it does |
| --- | --- |
| **Play → Avatar Gauntlet** | Climb the 18-rung, 18-floor ladder of named boss opponents, reshuffled daily with floor-scaled difficulty; clear a rung and roll straight into the next, with per-rung gold and a completion bonus. |
...
| **Play → Practice** | A one-off duel with no ladder attached: pick any of the 18 tower bosses (with their deck and personality) or a plain Easy / Medium / Hard opponent. |
| **Shop** | Buy a 9-card booster (Core Set, Ragnarök, Celtic Fae, Arthurian Court, Gothic Monsters, or Dark Tales) and watch the rarity/frame/holo reveal animate slot by slot, or buy whole decks (the unpicked starters and the expansion precons) from the Decks tab, each with a full-stats preview. Every booster carries an info glyph with that set's exact drop rates. |
...
That separation is what makes a real test suite possible: **1,072 tests (4 skipped) across 114 files**, covering engine flow/combat/keywords/mana/RNG/determinism, the stack and effects, catalog integrity, meta systems (collection, economy, save migrations, gauntlet, achievements, daily quests, Limited drafting, deck share codes, deck color identity), the variant/drop-distribution math behind the booster system, economy EV gates and named exploit regressions backed by a 10-persona progression simulator, onboarding tutorial determinism, audio recipes and music patterns, platform/gesture/render-scale behavior, and AI smoke tests plus the win-rate gates above (hundreds of full AI-vs-AI games). The whole suite finishes in about three minutes on the release-prep Windows host.
...
**Coming after 1.4:** the **"Darlings"** commander-style format, the **Cyberpunk Yokai Nights** expansion, variant deck building, the Dark Tales power pass, save export/import codes, and possible persona-metagame CI promotion in 1.5. Suggested decks, player-facing replays, Story Mode, later expansions, and player-made card packs follow the 1.5-to-2.0 slate. Real head-to-head LAN multiplayer is designed but remains further out.
