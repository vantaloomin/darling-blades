...
const R: CardDef[] = [
  ...
  charm('dt-briar-rose-lullaby', 'Briar-Rose Lullaby', {
    ...
    retell: { cost: cost(2, 'U') }, rarity: 'r', flavor: 'The melody softens the thorns without making them kinder.',
  }),
  ...
  ritual('dt-bayou-masquerade', 'Bayou Masquerade', {
    ...
    retell: { cost: cost(6, 'B') }, rarity: 'r', flavor: 'The masks come off when the fireflies start counting.',
  }),
...
const C: CardDef[] = [
  ...
  creature('dt-red-cloak-runner', 'Red-Cloak Runner', ['Human', 'Hunter'], { cost: cost(1, 'R'), colors: ['R'], attack: 2, defense: 1, keywords: ['warcry'], rarity: 'c', flavor: 'The path is short when you refuse to look behind you.' }),
  ...
  make('dt-satin-slipper', 'Satin Slipper', ['ritual'], ['Relic'], { cost: cost(1), colors: [], skim: { cost: cost(1) }, abilities: [spell([{ op: 'gainLife', n: 1 }])], rarity: 'c', flavor: 'Soft enough for dancing, hard enough for one last step.' }),
  charm('dt-page-torn-free', 'Page Torn Free', { cost: cost(2, 'U'), colors: ['U'], abilities: [spell([{ op: 'draw', n: 1 }])], retell: { cost: cost(2, 'U') }, rarity: 'c', flavor: 'The missing page is where the story starts behaving.' }),
  charm('dt-once-more-with-magic', 'Once More With Magic', { cost: cost(0, 'W'), colors: ['W'], abilities: [spell([{ op: 'boost', p: 1, t: 1, scope: 'target' }], target('creature'))], retell: { cost: cost(2, 'W') }, rarity: 'c', flavor: 'The second chance comes with better timing.' }),
  ritual('dt-wicked-step', 'Wicked Step', { cost: cost(0, 'B'), colors: ['B'], abilities: [spell([{ op: 'discardRandom', n: 1, who: 'opponent' }])], rarity: 'c', flavor: 'The stair creaks for everyone but the person who pushed.' }),
  charm('dt-rose-vine-snare', 'Rose-Vine Snare', { cost: cost(0, 'G'), colors: ['G'], abilities: [spell([{ op: 'boost', p: 2, t: 2, scope: 'target' }], target('creature'))], rarity: 'c', flavor: 'The vine catches, then decides it likes the shape.' }),
  enchantment('dt-candle-in-window', 'Candle in the Window', [], { cost: cost(0, 'W'), colors: ['W'], abilities: [dawn([{ op: 'gainLife', n: 1 }])], rarity: 'c', flavor: 'A small light can make a long road possible.' }),
  artifact('dt-ink-black-carriage', 'Ink-Black Carriage', ['Vehicle'], { cost: cost(0, 'B'), colors: ['B'], abilities: [{ when: 'dawn', ops: [{ op: 'grind', n: 1, who: 'self' }, { op: 'loseLife', n: 1, who: 'opponent' }] }], rarity: 'c', flavor: 'It carries stories away from the people who started them.' }),
  charm('dt-sea-glass-knife', 'Sea-Glass Knife', { cost: cost(0, 'U'), colors: ['U'], abilities: [spell([{ op: 'recall', to: 'target' }], target('creature'))], rarity: 'c', flavor: 'Pretty glass, practical edge, no landward warranty.' }),
  ritual('dt-ash-sweep', 'Ash Sweep', { cost: cost(1, 'R'), colors: ['R'], abilities: [spell([{ op: 'damage', n: 2, to: 'target' }, { op: 'grind', n: 2, who: 'self' }], target('any'))], rarity: 'c', flavor: 'The hearth clears more than dust.' }),
  ...
  artifact('dt-wolf-path', 'Wolf-Path Charm', [], { cost: cost(0, 'G'), colors: ['G'], activated: { cost: { tap: true }, ops: [{ op: 'gainLife', n: 1 }] }, rarity: 'c', flavor: 'The safest road is the one the wolf has not noticed.' }),
  artifact('dt-palace-steps', 'Glass Slipper', [], { cost: cost(0, 'W'), colors: ['W'], activated: { cost: { tap: true, mana: cost(1) }, ops: [{ op: 'gainLife', n: 2 }] }, rarity: 'c', flavor: 'Every guest climbs the steps. Not every guest reaches the ballroom.' }),
  artifact('dt-midnight-road', 'Midnight Invitation', [], { cost: cost(0, 'B'), colors: ['B'], activated: { cost: { tap: true }, ops: [{ op: 'grind', n: 1, who: 'self' }] }, rarity: 'c', flavor: 'The road is empty because the invitation was accepted elsewhere.' }),
  artifact('dt-sea-cave', 'Sea-Cave Pearl', [], { cost: cost(0, 'U'), colors: ['U'], activated: { cost: { tap: true }, ops: [{ op: 'foresee', n: 1 }] }, rarity: 'c', flavor: 'Foam hides the entrance and the price of leaving.' }),
  artifact('dt-hearth-cinders', 'Banked Cinders', [], { cost: cost(1, 'R'), colors: ['R'], activated: { cost: { tap: true }, ops: [{ op: 'damage', n: 1, to: 'opponent' }] }, rarity: 'c', flavor: 'The fire is out, but the coals are still warm enough to bite.' }),
  charm('dt-dream-prick', 'Dream Prick', { cost: cost(0, 'U'), colors: ['U'], abilities: [spell([{ op: 'tap', to: 'target' }, { op: 'grind', n: 1, who: 'self' }], target('creature'))], rarity: 'c', flavor: 'A dream can be interrupted without being forgotten.' }),
  charm('dt-rose-petal-shield', 'Rose-Petal Shield', { cost: cost(0, 'W'), colors: ['W'], abilities: [spell([{ op: 'boost', p: 0, t: 2, scope: 'target' }], target('creature'))], retell: { cost: cost(2, 'W') }, rarity: 'c', flavor: 'The petals turn aside what the thorns cannot.' }),
  make('dt-singing-shell', 'Singing Shell', ['ritual'], ['Relic'], { cost: cost(0, 'U'), colors: ['U'], skim: { cost: cost(1) }, abilities: [spell([{ op: 'foresee', n: 1 }, { op: 'grind', n: 1, who: 'self' }])], rarity: 'c', flavor: 'Put it to your ear and it tells you where the current turns.' }),
  ...
  ritual('dt-palace-masquerade', 'Palace Masquerade', { cost: cost(2, 'W'), colors: ['W'], abilities: [spell([{ op: 'createToken', token: 'tok-masked-guest', count: 2 }, { op: 'foresee', n: 1 }])], rarity: 'c', flavor: 'The guest list is a spell with excellent manners.' }),
  make('dt-ragged-ballgown', 'Ragged Ballgown', ['ritual'], ['Relic'], { cost: cost(1), colors: [], skim: { cost: cost(1) }, abilities: [spell([{ op: 'gainLife', n: 2 }])], rarity: 'c', flavor: 'The hem is torn, but the entrance is still worth making.' }),
  ...
  charm('dt-lullaby-refrain', 'Lullaby Refrain', { cost: cost(0, 'U'), colors: ['U'], abilities: [spell([{ op: 'tap', to: 'target' }], target('creature'))], retell: { cost: cost(2, 'U') }, rarity: 'c', flavor: 'The refrain returns when the sleeper thinks it is over.' }),
  make('dt-apple-basket', 'Apple Basket', ['ritual'], ['Relic'], { cost: cost(0, 'G'), colors: ['G'], skim: { cost: cost(1) }, abilities: [spell([{ op: 'gainLife', n: 3 }])], rarity: 'c', flavor: 'The basket is full, the orchard is quiet, and the deal is unclear.' }),
  ...
  artifact('dt-winter-bridge', 'Winter-Bridge Toll', [], { cost: cost(0, 'U'), colors: ['U'], activated: { cost: { tap: true }, ops: [{ op: 'severGrave', n: 1, who: 'opponent' }] }, rarity: 'c', flavor: 'The bridge is clear until the palace decides otherwise.' }),
  ritual('dt-palace-market-chase', 'Palace-Market Chase', { cost: cost(1, 'R'), colors: ['R'], abilities: [spell([{ op: 'damage', n: 2, to: 'target' }], target('any'))], skim: { cost: cost(1) }, rarity: 'c', flavor: 'The market parts for a runner with a good enough story.' }),
  make('dt-brass-lamp-charm', 'Brass Lamp Charm', ['ritual'], ['Relic'], { cost: cost(1), colors: [], skim: { cost: cost(1) }, abilities: [spell([{ op: 'foresee', n: 1 }])], rarity: 'c', flavor: 'Polish the lamp, then decide which wish can survive daylight.' }),
  artifact('dt-desert-rooftop', 'Rooftop Spyglass', [], { cost: cost(0, 'R'), colors: ['R'], activated: { cost: { tap: true, mana: cost(1) }, ops: [{ op: 'foresee', n: 2 }] }, rarity: 'c', flavor: 'The city roof catches moonlight and runaway wishes.' }),
  ritual('dt-reflection-sword', 'Reflection Sword', { cost: cost(0, 'W'), colors: ['W'], abilities: [spell([{ op: 'boost', p: 1, t: 0, keywords: ['firstBlade'], scope: 'allYours' }])], rarity: 'c', flavor: 'The blade reflects the family it expects you to become.' }),
  charm('dt-training-yard-dawn', 'Training-Yard Dawn', { cost: cost(0, 'W'), colors: ['W'], abilities: [spell([{ op: 'boost', p: 1, t: 1, scope: 'target' }, { op: 'foresee', n: 1 }], target('creature'))], rarity: 'c', flavor: 'Practice becomes a promise when the sun comes up.' }),
  charm('dt-ancestor-smoke', "Ancestor's Smoke", { cost: cost(0, 'W'), colors: ['W'], abilities: [spell([{ op: 'foresee', n: 2 }])], retell: { cost: cost(4, 'W') }, rarity: 'c', flavor: 'The smoke curls toward the answer your family avoided.' }),
  artifact('dt-bayou-lantern', 'Bayou Lantern', ['Relic'], { cost: cost(1, 'G'), colors: ['G'], skim: { cost: cost(1) }, abilities: [dawn([{ op: 'gainLife', n: 2 }])], rarity: 'c', flavor: 'The lantern keeps the path visible and the mosquitoes interested.' }),
  artifact('dt-crescent-cookpot', 'Crescent Cookpot', ['Relic'], { cost: cost(2), colors: [], abilities: [arrives([{ op: 'foresee', n: 1 }]), dawn([{ op: 'gainLife', n: 1 }])], rarity: 'c', flavor: 'The recipe is older than the kitchen and twice as patient.' }),
  land('dt-riverboat-kitchen', 'Riverboat Kitchen', ['G', 'B'], 'c', 'The galley drifts while the stew and the story both reduce.'),
  artifact('dt-wayfinder-oar', 'Wayfinder Oar', ['Relic'], { cost: cost(1, 'U'), colors: ['U'], skim: { cost: cost(1) }, abilities: [arrives([{ op: 'foresee', n: 2 }]), dawn([{ op: 'grind', n: 1, who: 'self' }])], rarity: 'c', flavor: 'The oar points where the water wants to go next.' }),
  charm('dt-lagoon-current', 'Lagoon Current', { cost: cost(1, 'U'), colors: ['U'], abilities: [spell([{ op: 'recall', to: 'target' }, { op: 'foresee', n: 1 }], target('creature'))], rarity: 'c', flavor: 'The current nudges every problem toward another shore.' }),
  land('dt-oceanic-islet', 'Oceanic Islet', ['U', 'G'], 'c', 'A green island rises from water bright enough to mislead a star.'),
  ritual('dt-windblown-leaf-paint', 'Windblown Leaf-Paint', { cost: cost(1, 'G'), colors: ['G'], abilities: [spell([{ op: 'foresee', n: 2 }, { op: 'gainLife', n: 2 }])], rarity: 'c', flavor: 'The wind edits the painting into a map.' }),
  artifact('dt-riverbend-trail', 'Riverbend Waterwheel', [], { cost: cost(2, 'G'), colors: ['G'], activated: { cost: { tap: true, mana: cost(3) }, targets: [{ what: 'yourGraveCreature' }], ops: [{ op: 'reclaim' }] }, rarity: 'c', flavor: 'The wheel turns the river and every sensible conclusion.' }),
  charm('dt-plaid-arrow', 'Plaid Arrow', { cost: cost(0, 'G'), colors: ['G'], abilities: [spell([{ op: 'boost', p: 1, t: 1, keywords: ['wardingGaze'], scope: 'target' }], target('creature'))], rarity: 'c', flavor: 'The pattern is loud so the shot can be quiet.' }),
  make('dt-casita-door-charm', 'Casita Door Charm', ['ritual'], ['Relic'], { cost: cost(1, 'W'), colors: ['W'], abilities: [spell([{ op: 'createToken', token: 'tok-hearth-spirit', count: 1 }, { op: 'foresee', n: 1 }])], rarity: 'c', flavor: 'The door opens for the person the house was waiting to meet.' }),
  make('dt-jade-dragon-scale', 'Jade Dragon Egg', ['ritual'], ['Relic'], { cost: cost(1), colors: ['G'], skim: { cost: cost(1) }, abilities: [spell([{ op: 'foresee', n: 1 }])], rarity: 'c', flavor: 'The shell sleeps. What curls inside remembers being a storm.' }),
];
