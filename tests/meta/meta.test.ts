...
describe('SaveManager', () => {
  ...
  it('reads a save left under the legacy waifutcg key (rename survival)', () => {
    ...
    const b = new SaveManager(storage);
    expect(b.data.version).toBe(CURRENT_SAVE_VERSION);
...
describe('save migration old blobs ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ current schema', () => {
  ...
  it('migrates a v6 blob to the current schema: confirmDestructive defaults on, an explicit choice survives', () => {
    ...
    expect(m2.data.version).toBe(CURRENT_SAVE_VERSION);
  ...
  it('migrates a v23 blob to the current schema: confirmNoBlock defaults lethal-only and preserves a valid choice', () => {
    const base = freshSave(1);
    const v23settings = { ...base.settings } as Record<string, unknown>;
    delete v23settings.confirmNoBlock;

    const storage = fakeStorage();
    storage.raw.set('darlingblades.save.v1', JSON.stringify({ ...base, version: 23, settings: v23settings }));
    const migrated = new SaveManager(storage);
    expect(migrated.data.version).toBe(CURRENT_SAVE_VERSION);
    expect(migrated.data.settings.confirmNoBlock).toBe('lethal');

    const explicit = fakeStorage();
    explicit.raw.set(
      'darlingblades.save.v1',
      JSON.stringify({ ...base, version: 23, settings: { ...v23settings, confirmNoBlock: 'always' } }),
    );
    expect(new SaveManager(explicit).data.settings.confirmNoBlock).toBe('always');

    const malformed = fakeStorage();
    malformed.raw.set(
      'darlingblades.save.v1',
      JSON.stringify({ ...base, version: 23, settings: { ...v23settings, confirmNoBlock: 'later' } }),
    );
    expect(new SaveManager(malformed).data.settings.confirmNoBlock).toBe('lethal');
  });
  ...
  it('migrates a v9 blob to v10: tutorialDone is coerced from the win/loss record', () => {
    ...
    expect(mFresh.data.version).toBe(CURRENT_SAVE_VERSION);
  ...
  it('migrates a v15 blob forward: backfills draft personas and safely drops an active sealed run', () => {
    ...
    expect(migratedDraft.data.version).toBe(CURRENT_SAVE_VERSION);
    ...
    expect(migratedSealed.data.version).toBe(CURRENT_SAVE_VERSION);
