...
only, and `Game.submit` binds it to the card there, which is the card the
recorded game chose. A current-version log containing the explicit legacy
...
plates), with the top half rebuilt as a **point-mirror of the player's half**
in the UI-refresh Wave 1.5a (plan-ui-ux-refresh.md). Layout regions (the
...
- **Lands have no board tiles** (2026-07-10 board feedback; the old
  ...
  Per-basic saved-deck land styles render only for the human side; opponents
  and replay playback always use default land art.
