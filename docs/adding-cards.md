...
- **A non-creature permanent must do something after it arrives.** One-time effects belong on Rituals or Charms; an Artifact or Enchantment whose only battlefield text is an arrival trigger is a spell that leaves a blank object behind (Chrome Medallion shipped that way in 1.7.0 and was reworked in 1.7.1). Ongoing text means a non-arrival trigger, a mana ability, Quest chapters, Hauntlink, or a static keyword, and `tests/data/arrivalOnlyPermanents.test.ts` enforces it. A creature's body counts as ongoing; Skim does not, because it is a hand ability that never reaches the battlefield. Foresee 1 alone still may not be a card's only functional line: when a common wants a small arrival effect, vary it across a life point, a token, a graveyard clause, or a self-grind so no single line becomes a set's texture.

**The v1 laws** (stated at the top of `types.ts` and enforced throughout):

- **Only arrival triggers target.** Their mandatory choice is deferred through
  `pendingDecisions`; a trigger with no legal target fizzles silently.
- **Spell targets are single-target unless `upTo: 2` is declared**. The
  interpreter applies target effects once to each independently chosen target.
  Arrival triggers may defer one mandatory target choice. Other trigger kinds
  remain target-free.
...
| `damage`                                     | `{ n: number\|'X'; to: 'eachCreature'; severOnDeath?: true }`                                                     | `n` (or `ctx.x` if `'X'`) damage. `target` → `targets[0]` (player = life loss, creature = marked damage); `opponent`/`controller` → that player's face.                                                                                                                                                                                                                                                                                                         | `lifeChanged` / `damageMarked` |
...
| `destroyArtifactOrSeverEnchantment`          | `{ to: 'target'; }`                                                                                               | Branch on the target: an artifact is destroyed (graveyard); an enchantment is severed (artifact checked first for multi-typed permanents). Targets `artifactOrEnchantment`.                                                                                                                                                                                                                                                                                     | —                              |
| `cancel`                                     | `{ to: 'target' }`                                                                                                | **Targets a stack item** (`targets[0]` is a `stackItem`); removes it to its controller's graveyard.                                                                                                                                                                                                                                                                                                                                                             | `spellCountered`               |
| `boost`                                      | `{ p: number; t: number; keywords?: Keyword[]; scope: 'target'\|'allYours'\|'all'\|'yourMarked'\|'theirMarked' }` | Until-end-of-turn `+p/+t` (+ keywords). `target` → `targets[0]`; `allYours` → every creature you control.                                                                                                                                                                                                                                                                                                                                                       | (via stat recompute)           |
| `addCounters`                                | `{ n: number; to: 'target'\|'self' }`                                                                             | Puts `n` `+1/+1` Marks on the target creature (`target`) or source creature (`self`, via `ctx.sourceIid`). Noncreature targets are ignored by the creature-scoped mark system.                                                                                                                                                                                                                                                                                  | (via stat recompute)           |
...
| `removeMarks`                                | `{ to: 'target' }`                                                                                                | Removes every mark from the target creature.                                                                                                                                                                                                                                                                                                                                                                                                                    | (via stat recompute)           |
| `markAll`                                    | `{ scope: 'yourCreatures' }`                                                                                      | Adds one mark to each creature you control, firing mark observers per creature.                                                                                                                                                                                                                                                                                                                                                                                 | (via mark triggers)            |
...
| `ifTargetMarked`                             | `{ then: EffectOp[]; else?: EffectOp[] }`                                                                         | Runs the then branch when the target creature has a Mark, otherwise the optional else branch.                                                                                                                                                                                                                                                                                                                                                                   | (nested ops)                   |
...
| `foresee`                                    | `{ n: number; who?: 'targetOwner' }`                                                                              | Foresee `n`: look at the top `n` cards of your deck, bottom any subset, keep the rest in order (a deferred decision via `pendingDecisions`; revealed cards are redacted from the opponent's view)                                                                                                                                                                                                                                                               | —                              |
