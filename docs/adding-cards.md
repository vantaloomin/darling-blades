...
| `gainsMark`              | when this permanent gets a mark.                          |
| `yourPermanentMarked`    | whenever a permanent you control becomes marked.          |
| `youAddMark`              | whenever you add a mark to a permanent.                   |
...
| -------------------------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------ |
| `damage`                                     | `{ n: number\|'X'; to: 'target'\|'opponent'\|'controller'\|'eachCreature' }`                       | `n` (or `ctx.x` if `'X'`) damage. `target` → `targets[0]` (player = life loss, creature = marked damage); `opponent`/`controller` → that player's face.                                                                                                                                                                                                                                                                                                            | `lifeChanged` / `damageMarked` |
...
| `boost`                                      | `{ p: number; t: number; keywords?: Keyword[]; scope: 'target'\|'allYours'\|'all'\|'yourMarked' }` | Until-end-of-turn `+p/+t` (+ keywords). `target` → `targets[0]`; `allYours` → every creature you control.                                                                                                                                                                                                                                                                                                                                                          | (via stat recompute)           |
...
| `propagate`                                  | `{}`                                                                                               | Adds exactly ONE `+1/+1` mark to every permanent `ctx.controller` controls that already has `plusOneCounters > 0`. Creates nothing — a permanent at 0 marks is skipped — so it can only compound a marked board, never start one. `permanent`, not creature: a marked artifact, enchantment or land grows too. Takes no target and is target-free (legal as a Foresee continuation).                                                                               | (via stat recompute)           |
| `moveMark`                                   | `{}`                                                                                               | Moves one mark from the first chosen permanent you control to a different second permanent you control. The destination can receive its first mark.                                                                                                                                                                                                                                                                                                                | (via mark triggers)            |
...
| `ifTargetMarked`                             | `{ then: EffectOp[]; else?: EffectOp[] }`                                                          | Runs the then branch when the target has a mark, otherwise the optional else branch.                                                                                                                                                                                                                                                                                                                                                                               | (nested ops)                   |
...
| `destroyNewestOpponentArtifactOrEnchantment` | `{}`                                                                                               | Trigger-safe creature-stapled removal: destroys the opponent's most recently entered artifact or enchantment; no-op when none exists. No targeting, seeded-deterministic.                                                                                                                                                                                                                                                                                          | —                              |
