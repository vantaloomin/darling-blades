<!-- source-of-truth: src/data/cards/*.ts, src/art/PlaceholderArtGenerator.ts, src/ui/CardView.ts, src/ui/fx/HoloEffects.ts · last-verified: 2026-07-08 -->
...
`ragnarok.md` (60 creatures, generated by `scripts/gen-ragnarok-artbible.ts` — same
template, facts computed from the card data). Every rule below is grounded in
...
| `constructs-and-tokens.md` | `src/data/cards/artifacts.ts` + `tokens.ts` | 13 (5 + 8) |
| `ragnarok.md` | `src/data/cards/ragnarok.ts` | 60 (generated) |
...
- [ ] Every creature id in the catalog appears **exactly once** across the nine
      files, in source-file order; per-file counts match the table in section 10
      (212 entries total — 152 base + 60 Ragnarök).
