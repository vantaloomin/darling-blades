<!-- source-of-truth: src/data/cards/greek.ts · last-verified: 2026-07-04 -->

# Darling Blades Art Bible — Greek Pantheon (`gk`)

The Olympians read as one civilization: sun-bleached Pentelic **marble** (`#f2e8cf`),
**Aegean blue** in sea, sky, and dyed wool, and **gilt** trim on every hem, cuff, and
laurel. Costume language is the draped chiton, himation, and peplos — pleated cloth
that catches rim light and holds its integrity in motion — over sandaled greaves and
worked-bronze accents. Architecture is fluted colonnade, coffered ceiling, and open
horizon; nature is olive, cypress, wine-dark sea. Each goddess owns one signature
attribute (aegis, thunderbolt, silver bow, poppy, trident) rendered as a real forged
or grown object, never a glowing abstraction. Goddesses appear as-is; gods are
genderbent but kit-faithful. The faction accents — marble, Aegean blue, gilt — layer
over each card's color-identity anchor and never replace it.

### Athena, Aegis of Wisdom — `gk-athena`
- **Card facts:** {2}{W}{W} · W · 2/4 · vigilance · ur, legendary · holo: foil (explicit)
- **Character & source:** Athena as-is, goddess of strategic war; her static aura toughens every other Olympian, so she reads as the anchor the pantheon forms up behind.
- **Personality / mood:** "Wisdom is knowing which wars not to fight. Strategy is winning those anyway." Calm, measured, three moves ahead — a general who has already read the battle's ending.
- **Pose & composition:** Standing three-quarter, weight settled, aegis presented across the body rather than raised to strike (2/4 vigilance defends, it does not lunge); face ≈ y 320, eyes tracking something off-frame she has already accounted for. Owl perched on the shoulder in the band's upper right.
- **Costume & attire:** White-and-gold pleated peplos, bronze-scaled aegis mantle with the gorgoneion boss at the shoulder, crested Corinthian helm pushed back off the brow, greaves.
- **Palette:** W palette (`#f2e8cf` → `#c9a84c`, accent `#fffef2`) with marble whites and gilt scale-trim; Aegean-blue crest plume. Greek marble + gilt accents.
- **Lighting:** Cool marble ambient fill as key; warm gilt rim off an off-frame torch lining the helm and aegis rim.
- **Expression:** Serene, appraising, unhurried; a strategist's faint half-attention.
- **Props / weapon:** The scaled aegis with gorgoneion, upright spear held loosely, companion owl.
- **Background:** Fluted colonnade opening onto a hazed muster-field, formed ranks implied in soft focus behind her.
- **Holo interaction:** Foil — build texture variance into the aegis scales, gilt embroidery, and helm crest so the rainbow bands land there as forged shimmer; keep her face and the flat marble field quiet.
- **Rarity ambition:** Rare "moment" — the still second before the order to advance.
- **Prompt:** Athena in white-and-gold peplos and bronze scaled aegis mantle with gorgoneion boss, crested Corinthian helm pushed back, upright spear and shoulder owl, marble colonnade over a hazed muster-field, serene strategist gazing off-frame, cool marble fill with warm gilt rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Ares, Crimson Havoc — `gk-ares`
- **Card facts:** {2}{R}{R} · R · 4/3 · haste · sr, legendary · holo: auto:radial
- **Character & source:** Genderbent Ares, god of slaughter; 4/3 haste means she is already inside your line the turn she lands, kit-faithful with spear and round hoplon.
- **Personality / mood:** "She arrives uninvited and leaves nothing standing to complain." Joyous, indiscriminate violence — war as appetite, not policy.
- **Pose & composition:** Mid-charge low-angle three-quarter, spear cocked back to thrust, hoplon swung wide, face ≈ y 330 turned to the viewer mid-stride; the spearhead breaks the top of the band. Motion drag on the trailing crimson cloak.
- **Costume & attire:** Blood-red pleated war-kilt over blackened bronze cuirass, high crested helm streaming a crimson horsehair plume, gilt greaves, torn battle-sash.
- **Palette:** R palette (`#d95436` → `#5e0f0f`, accent `#f7b267`) pushed hot — ember reds, near-black bronze, one cold steel note on the spearhead. Greek marble + gilt accents on cuirass edging.
- **Lighting:** Low battlefield firelight as key; cold steel rim off the spear and helm crest.
- **Expression:** Bared-teeth grin, eyes blown wide with the joy of it.
- **Props / weapon:** Long thrusting spear (dory) and round hoplon shield blazoned with a spear-sun.
- **Background:** A burning field behind, smoke and sparks; a centered ember-glow bloom low in the frame.
- **Holo interaction:** Radial — reward it with the centered fire bloom low-center so the concentric rainbow rings breathe outward from the flames; keep her lit face bright above the ring source.
- **Rarity ambition:** Rare — the instant of the first strike landing.
- **Prompt:** Genderbent Ares mid-charge in blood-red war-kilt and blackened bronze cuirass, crimson-plumed crested helm, long spear cocked to thrust and round hoplon shield, burning battlefield with centered ember glow, grinning war-goddess lit from below, cold steel rim light — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Artemis of the Silver Bow — `gk-artemis`
- **Card facts:** {2}{G} · G · 3/3 · reach · r · holo: auto:sheen
- **Character & source:** Artemis as-is, huntress of the wilds; reach reads as a nocked arrow that can pull anything out of the sky.
- **Personality / mood:** "Nothing on wings escapes her notice. Little on legs does either." Focused, cold-patient, wholly self-possessed.
- **Pose & composition:** Half-drawn kneeling-to-standing stance, bow raised and tracking a high target, sightline and arrow angled into the upper band; face ≈ y 330 in profile-three-quarter, eye on the shaft. The environmental story element (uncommon): a bounding deer silhouette in the far midground she has already ranged.
- **Costume & attire:** Short forest-green hunting chiton belted for movement, silver crescent diadem, quiver and baldric, leather bracer, sandaled greaves.
- **Palette:** G palette (`#4fa06a` → `#123a22`, accent `#a9dcae`) — moss and deep pine greens with a cold silver bow and diadem; Aegean-blue dusk beyond. Greek marble + gilt on the quiver caps.
- **Lighting:** Cool moonlit key from the target's direction; warm low rim off a distant campfire.
- **Expression:** Narrowed, unblinking hunter's focus.
- **Props / weapon:** The silver recurve bow at half-draw, feathered arrow, quiver.
- **Background:** Moonlit forest edge, one distant fleeing deer, cypress silhouettes.
- **Holo interaction:** Sheen — the Shine sweep passes cleanly over toned greens and silver; keep the moonlit sky slightly textured so the sweep doesn't white it out flat.
- **Rarity ambition:** Uncommon — one figure plus the ranged-and-doomed deer as story.
- **Prompt:** Artemis in short forest-green hunting chiton with silver crescent diadem, silver recurve bow at half-draw tracking a target skyward, quiver and baldric, moonlit forest edge with a distant fleeing deer, focused huntress in profile, cool moonlight key with warm distant rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Hestia, Hearthkeeper — `gk-hestia`
- **Card facts:** {1}{W} · W · 1/3 · lifelink · c · holo: none
- **Character & source:** Hestia as-is, goddess of the hearth; 1/3 lifelink is a warm, defensive presence — she tends, she does not strike.
- **Personality / mood:** "Every fire in every camp is a little bit hers." Gentle, steadying, quietly everywhere at once.
- **Pose & composition:** Kneeling-tending at a low brazier, cupping a small flame, chest-up composition, face ≈ y 340 tilted down toward the fire she keeps. One clear idea — warmth — readable at 119×86. No lunge.
- **Costume & attire:** Simple cream-white belted chiton, veiled himation drawn softly over the hair, plain gilt hearth-key at the belt.
- **Palette:** W palette (`#f2e8cf` → `#c9a84c`, accent `#fffef2`) — cream and soft gold, warmed by the flame's amber. Greek marble + gilt accents, minimal.
- **Lighting:** Hearth-fire uplight as warm key on face and hands; cool marble rim behind.
- **Expression:** Soft, maternal, eyes lowered and content.
- **Props / weapon:** A small tended brazier flame and the gilt hearth-key; no weapon.
- **Background:** Two-value dim interior — a hint of marble wall, nothing to distract from the flame.
- **Holo interaction:** none — common.
- **Rarity ambition:** Common — single figure, one idea (warmth), reads at thumbnail scale.
- **Prompt:** Hestia kneeling in cream-white belted chiton and softly veiled himation, cupping a small brazier flame with a gilt hearth-key at her belt, dim marble interior, gentle maternal goddess with lowered eyes, warm hearth uplight and cool rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Nike, Herald of Triumph — `gk-nike`
- **Card facts:** {1}{W} · W · 2/1 · flying · c · holo: none
- **Character & source:** Nike as-is, winged victory; a 2/1 flyer — light, fast, evasive, all upward momentum.
- **Personality / mood:** "Victory has wings and a very smug smile." Exultant, triumphant, a little insufferable about it.
- **Pose & composition:** Descending mid-air, wings spread, one arm sweeping a laurel wreath forward, face ≈ y 300 tilted slightly down at the viewer; the great wing-tips break the top and side of the band. One idea — winged victory — clean at 119×86.
- **Costume & attire:** White-and-gold flowing chiton streaming upward with the descent, gilt sandals, a golden laurel already crowning her hair.
- **Palette:** W palette (`#f2e8cf` → `#c9a84c`, accent `#fffef2`) — luminous white cloth, gilt feather-edges. Greek marble + gilt; a sliver of Aegean-blue sky.
- **Lighting:** Bright sky key from above-behind (halo effect on the wings); warm gilt rim on the wreath and hair.
- **Expression:** Radiant, smug, knowing smile.
- **Props / weapon:** The offered laurel wreath and her feathered wings; no weapon.
- **Background:** Open bright sky, a single distant marble victory-column below — kept simple.
- **Holo interaction:** none — common.
- **Rarity ambition:** Common — single figure, one idea (winged triumph).
- **Prompt:** Nike descending mid-air in streaming white-and-gold chiton, wings spread, sweeping a golden laurel wreath forward, gilt sandals and laurel crown, open bright sky over a distant marble column, radiant smug victory-goddess, bright sky key with warm gilt rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Shieldmaiden of Sparta — `gk-hoplite`
- **Card facts:** {1}{W} · W · 2/2 · firstStrike · c · holo: none
- **Character & source:** An original Spartan hoplite of the Olympian faction — disciplined line infantry, the pantheon's rank-and-file steel.
- **Personality / mood:** "Come back with your shield — she always does." Grim, dutiful, unshakable in the phalanx.
- **Pose & composition:** Braced set-stance behind a large round shield, spear leveled over the rim to strike first (firstStrike = spear already out and ready), face ≈ y 330 set hard over the shield's top edge. One idea — the wall that hits first — reads at thumbnail.
- **Costume & attire:** Crimson-cloaked over a bronze muscle cuirass, transverse-crested Corinthian helm, leather pteruges, greaves — classic Spartan silhouette.
- **Palette:** W palette (`#f2e8cf` → `#c9a84c`, accent `#fffef2`) — bronze and marble tones with a crimson cloak accent. Greek marble + gilt on cuirass edging.
- **Lighting:** Hard directional daylight key throwing a sharp shadow under the helm; cool sky rim on the bronze.
- **Expression:** Jaw set, eyes level and cold under the helm's shadow.
- **Props / weapon:** Leveled dory spear and a large lambda-blazoned hoplon shield.
- **Background:** Two-value dusty plain, the suggestion of an allied shield-line to one side.
- **Holo interaction:** none — common.
- **Rarity ambition:** Common — single figure, one idea (first-striking shield-wall).
- **Prompt:** Spartan shieldmaiden braced behind a large lambda-blazoned round shield with leveled spear over the rim, bronze muscle cuirass and transverse-crested Corinthian helm, crimson cloak, dusty plain with implied shield-line, grim disciplined warrior, hard daylight key with cool bronze rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Zeus, Thunder Empress — `gk-zeus`
- **Card facts:** {2}{R}{R}{W}{W} · R/W (gold frame) · 5/5 · flying · ur, legendary · holo: galaxy (explicit)
- **Character & source:** Genderbent Zeus, sovereign of Olympus; her ETB burns the opponent for 3, so the art must catch the bolt already loosed. Kit-faithful: the thunderbolt is her signature.
- **Personality / mood:** "The forecast is her mood. Pack accordingly." Imperious, electric, casually catastrophic.
- **Pose & composition:** High-hovering three-quarter, one arm hurling a forked thunderbolt down and across the frame, wings and storm-cloak thrown wide, face ≈ y 300 gazing down at the viewer; the bolt deliberately breaks the top-right of the band. Aloft, above the storm.
- **Costume & attire:** White-and-gold imperial chiton with a storm-grey himation, gilt aquila-eagle pauldron, laurel-and-lightning diadem, dark feathered wings.
- **Palette:** Gold palette (`#e8c95a` → `#7a5a18`, accent `#fff2b8`) carrying the red/white split — gilt regalia, ember-red bolt core, marble-white cloth. Greek marble + gilt; Aegean-blue storm.
- **Lighting:** The thunderbolt itself as blue-white key flaring the near planes; warm gilt rim on regalia.
- **Expression:** Sovereign, thunder-lit, faintly amused at the ruin below.
- **Props / weapon:** The forked thunderbolt (canon kit) and the aquila eagle wheeling beside her.
- **Background:** A dark, high-contrast storm sky, black thunderheads and a star-broken void above.
- **Holo interaction:** Galaxy — the nebula SCREEN-blends into the dark storm sky, making a churning cosmos; keep the upper field dark-valued, her face bright, and the bolt a clean specular edge for star glints.
- **Rarity ambition:** Gold-legendary storm-throne splash — box-art tier.
- **Prompt:** Genderbent Zeus hovering above a dark storm, white-and-gold imperial chiton with storm-grey himation and gilt eagle pauldron, hurling a forked thunderbolt down across frame, dark wings spread, black thunderheads and star-torn void, imperious storm-lit empress, blue-white bolt key with warm gilt rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Hera, Queen of Olympus — `gk-hera`
- **Card facts:** {2}{W}{B} · W/B (gold frame) · 3/4 · vigilance · ssr, legendary · holo: foil (explicit)
- **Character & source:** Hera as-is, Queen of Olympus; her ETB creates two Peacock tokens whose design **this art canonizes** (`tok-peacock` — see registry).
- **Personality / mood:** "Her peacocks have a hundred eyes each, and every eye is on Zeus." Absolute sovereignty with a surveillance budget.
- **Pose & composition:** Enthroned three-quarter seated, slightly above camera, chin high, face ≈ y 300; two peacocks flank her — one perched on the throne back, its tail cascading down the right edge of the band; feather eye-spots scattered through the midground; the crown may rise into the top bleed.
- **Costume & attire:** White-and-gold pleated chiton with a **black** silk himation (her B identity), golden peacock-feather brooch, polos crown, heavy cuffs.
- **Palette:** Gold palette (`#e8c95a` → `#7a5a18`, accent `#fff2b8`) carrying the white/black split; iridescent teal-sapphire reserved exclusively for the feather eye-spots. Greek marble + gilt accents.
- **Lighting:** Cool marble ambient fill + warm gilt key from an off-frame brazier; subtle underglow keeps the black silk reading as depth, not a hole.
- **Expression:** Serene, knowing, faintly pitying; direct eye contact.
- **Props / weapon:** The throne, the polos crown, and the two canon peacocks.
- **Background:** Olympian colonnade in haze, storm light far beyond — Zeus is out there, and she knows exactly where.
- **Holo interaction:** Foil — rainbow bands bloom on textured patches, so build texture into the peacock tail eye-spots, gold embroidery, and crown; keep her face and the black silk smooth so the **birds** do the shimmering under the pointer.
- **Rarity ambition:** Gold-legendary throne-scene hero shot.
- **Prompt:** Hera enthroned in white-and-gold chiton with black silk mantle and polos crown, two peacocks with luminous eye-spotted tails flanking a marble throne, Olympian colonnade with distant storm light, regal serene goddess gazing down at viewer, warm gilt key light on cool marble — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Aphrodite, Heart's Tide — `gk-aphrodite`
- **Card facts:** {1}{W}{U} · W/U (gold frame) · 2/3 · lifelink · ssr, legendary · holo: sparkle (explicit)
- **Character & source:** Aphrodite as-is, born of the sea-foam; lifelink and an upkeep gainLife make her a warm, replenishing presence — beauty that mends what it touches.
- **Personality / mood:** "Wars have started over less. Specifically, over her." Effortless, magnetic, wholly aware of the effect she has.
- **Pose & composition:** Rising three-quarter from cresting surf, chin lifted, one hand trailing water, face ≈ y 310 turned softly to the viewer; hair and foam sweep upward, a wave curl breaking into the upper band behind her. Serene, buoyant — no aggression.
- **Costume & attire:** Diaphanous white-and-sea-blue chiton clinging with spray, a golden shell-clasp at the shoulder, pearl strands, gilt sea-foam circlet.
- **Palette:** Gold palette (`#e8c95a` → `#7a5a18`, accent `#fff2b8`) carrying the white/blue split — pearl whites, Aegean-blue surf, gilt shell. Greek marble + gilt; Aegean blue is the sea itself.
- **Lighting:** Warm dawn key gilding the wave crests; cool sea-blue rim from the water below.
- **Expression:** Softly beguiling, half-smile, luminous eyes.
- **Props / weapon:** The golden shell, pearl strands, cresting foam; no weapon.
- **Background:** Sunlit open sea, backlit spray, a distant marble shoreline temple.
- **Holo interaction:** Sparkle — star-glint particles only; design as `none` but give the foam crests and pearls crisp bright speculars for the glints to echo across the water.
- **Rarity ambition:** Gold-legendary sea-birth splash.
- **Prompt:** Aphrodite rising from cresting surf in a diaphanous white-and-sea-blue chiton with a golden shell-clasp and pearl strands, gilt sea-foam circlet, spray sweeping upward, sunlit open sea and distant marble temple, beguiling half-smiling goddess, warm dawn key with cool sea-blue rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Persephone, Queen of Two Courts — `gk-persephone`
- **Card facts:** {2}{B}{G} · B/G (gold frame) · 3/3 · deathtouch · ssr, legendary · holo: radial (explicit)
- **Character & source:** Persephone as-is, sovereign of both the underworld and spring; her dies-trigger sows two Bloom tokens whose design **this art canonizes** (`tok-bloom` — see registry).
- **Personality / mood:** "Six months of paperwork below, six months of gardening above." Dual-natured, unbothered, quietly lethal — deathtouch means one graze is enough.
- **Pose & composition:** Standing three-quarter, one hand cradling a split pomegranate, the other resting on a single blooming vine (the deathtouch implement: a thorn she need not swing); face ≈ y 315, calm and centered. A halved dark/light composition — shadow-side and spring-side — meeting at her spine.
- **Costume & attire:** A gown split down the seam: deep obsidian-black underworld silk on one half, living-green vine-embroidered spring cloth on the other; pomegranate-red circlet, gilt seam-clasps.
- **Palette:** Gold palette (`#e8c95a` → `#7a5a18`, accent `#fff2b8`) carrying the black/green split — obsidian B (`#140d1c`) against moss G (`#4fa06a`), pomegranate-red blossom accent. Greek marble + gilt clasps.
- **Lighting:** Cold underworld underglow keying the black half; warm spring sun keying the green half — a centered seam of balanced light.
- **Expression:** Composed, faintly amused, unafraid of either court.
- **Props / weapon:** Split pomegranate, the single blooming (yet lethal) vine; the tok-bloom sprouts foreshadowed at her feet — pomegranate-red blossoms on green.
- **Background:** A vertical split: cavern-dark asphodel behind the black half, sunlit meadow behind the green.
- **Holo interaction:** Radial — reward it with a centered glow at the seam where light meets dark, so the concentric rainbow rings breathe outward from her core.
- **Rarity ambition:** Gold-legendary duality splash.
- **Prompt:** Persephone standing in a seam-split gown, obsidian underworld silk on one half and green spring vine-cloth on the other, cradling a split pomegranate over a blooming vine, pomegranate-red circlet, background split between cavern asphodel and sunlit meadow, composed dual goddess, cold underglow meeting warm spring key at center — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Hades, Unseen Sovereign — `gk-hades`
- **Card facts:** {3}{B}{B} · B · 5/4 · deathtouch · ssr, legendary · holo: sheen (explicit)
- **Character & source:** Genderbent Hades, ruler of the dead; her ETB drains 3 life from the opponent — a schemer who advances appointments. Deathtouch: one touch settles the account.
- **Personality / mood:** "Everyone arrives eventually. She just moves up appointments." Cold, patient, administratively inevitable.
- **Pose & composition:** Enthroned or standing three-quarter, one hand extended palm-up summoning a wisp of a soul (the ETB drain in motion), the bident resting; face ≈ y 305, half-lit, gazing levelly at the viewer. She schemes, she does not lunge.
- **Costume & attire:** Deep obsidian-black chiton and shadow-himation, a dull-gilt Helm of Darkness pushed back, oil-black gemstone regalia, gilt seam-trim.
- **Palette:** B palette (`#5a3a70` → `#140d1c`, accent `#9b6fc4`) — obsidian and cold amethyst, one thin gilt line. Greek marble + gilt, kept sparse and tarnished.
- **Lighting:** Cold amethyst underglow from the summoned soul-wisp as key; a single warm gilt rim tracing her helm and jaw.
- **Expression:** Impassive, unhurried, the faintest downturn of finality.
- **Props / weapon:** The two-tined bident, the Helm of Darkness, a drifting soul-wisp in her palm.
- **Background:** The dim asphodel throne-hall, columns dissolving into black, a far Styx-glow.
- **Holo interaction:** Sheen — the Shine sweep passes over toned obsidian and gilt; keep the large black fields slightly textured (worn silk, gemwork) so the sweep reads as sheen, not a white wash.
- **Rarity ambition:** Rare "moment" — a soul being drawn up mid-summons.
- **Prompt:** Genderbent Hades in obsidian chiton and shadow himation with a dull-gilt Helm of Darkness, holding a two-tined bident, a cold soul-wisp drawn up from her open palm, dim asphodel throne-hall with far Styx-glow, impassive sovereign of the dead, cold amethyst underglow with a single gilt rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Poseidon, Tidal Empress — `gk-poseidon`
- **Card facts:** {3}{U}{U} · U · 5/5 · ssr, legendary · holo: foil (explicit)
- **Character & source:** Genderbent Poseidon, empress of the sea; her ETB draws a card — the tide surrenders its secrets. Kit-faithful: the trident is her signature.
- **Personality / mood:** "The sea remembers every slight. So does she. It is the same list." Vast, grudge-keeping, sovereign of a whole element.
- **Pose & composition:** Rising commanding three-quarter from a wall of surf, trident raised to summon, face ≈ y 305 gazing down at the viewer; a great wave curls up and breaks into the top band behind her. Immense presence, not a charge.
- **Costume & attire:** Deep sea-blue-and-teal scaled chiton, coral-and-gilt pauldrons, a crown of wave-crests, kelp-silk mantle, pearl regalia.
- **Palette:** U palette (`#4a90d9` → `#16294f`, accent `#a8d4f7`) — Aegean and abyssal blues, foam-white crests, gilt coral trim. Greek marble + gilt; Aegean blue is her domain.
- **Lighting:** Cool aqueous key filtering through the wave; warm gilt rim off the trident tines and crown.
- **Expression:** Imperious, storm-eyed, ancient grievance held steady.
- **Props / weapon:** The three-pronged trident (canon kit) and the summoned wave-wall.
- **Background:** Towering storm-sea, a sunken marble colonnade breaking the surface behind her.
- **Holo interaction:** Foil — build texture variance into the scaled chiton, coral pauldrons, and wave-crests so the rainbow bands land as wet-scale shimmer; keep her face and any flat dark water quiet.
- **Rarity ambition:** Rare "moment" — the sea rising at her command.
- **Prompt:** Genderbent Poseidon rising from a wall of surf in sea-blue scaled chiton with coral-and-gilt pauldrons and a wave-crest crown, raising a three-pronged trident, a great wave curling behind, storm-sea over sunken marble colonnade, imperious sea-empress, cool aqueous key with warm gilt trident rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Apollo, Radiant Muse — `gk-apollo`
- **Card facts:** {1}{W}{W} · W · 2/2 · r · holo: auto:sheen
- **Character & source:** Genderbent Apollo, god of light and music; her static aura brightens every other Olympian (+1/+1), so she reads as the pantheon's tuning-fork. Kit-faithful: the lyre.
- **Personality / mood:** "Her lyre tunes the whole pantheon half a step brighter." Luminous, effortless, radiant self-assurance.
- **Pose & composition:** Three-quarter mid-strum, lyre lifted, head tilted into her own music, face ≈ y 320; a warm sun-disc glows behind her head. Environmental story element (uncommon): golden note-motes drifting off the strings through the band.
- **Costume & attire:** White-and-gold chiton with a sunburst himation, gilt laurel crown, sun-disc pendant, sandaled greaves.
- **Palette:** W palette (`#f2e8cf` → `#c9a84c`, accent `#fffef2`) — radiant whites and heavy gilt, warm golden light. Greek marble + gilt accents.
- **Lighting:** Warm sun-disc backlight as key (halo bloom); soft gilt rim on the lyre and laurel.
- **Expression:** Serene rapture, eyes half-closed in the music.
- **Props / weapon:** The golden lyre and drifting note-motes; no weapon.
- **Background:** Sunlit marble sanctuary, a glowing sun-disc centered behind her.
- **Holo interaction:** Sheen — the Shine sweep glides across gilt and warm cloth; keep the bright sun-disc field textured (rays, note-motes) so the sweep doesn't blow it flat white.
- **Rarity ambition:** Uncommon — one figure plus the drifting music-motes as story.
- **Prompt:** Genderbent Apollo mid-strum on a golden lyre in white-and-gold chiton with a sunburst himation and gilt laurel crown, glowing sun-disc behind her head, golden note-motes drifting from the strings, sunlit marble sanctuary, radiant rapt muse, warm backlit sun-disc key with soft gilt rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Hermes, Gale-Heeled Courier — `gk-hermes`
- **Card facts:** {1}{U} · U · 2/1 · flying, haste · r · holo: auto:sheen
- **Character & source:** Genderbent Hermes, messenger of the gods; flying + haste means she is gone the turn she arrives. Kit-faithful: winged sandals and caduceus.
- **Personality / mood:** "Delivered your defeat notice yesterday. You'll receive it tomorrow." Quick-witted, sly, always three errands ahead.
- **Pose & composition:** Full-tilt airborne dash across the frame, one winged sandal kicked back, caduceus thrust forward, face ≈ y 320 grinning back over the shoulder; motion drag on the mantle, wing-tips breaking the band's sides. Pure velocity.
- **Costume & attire:** Sky-blue-and-white short traveling chiton belted high, winged petasos hat, winged sandals, courier's sling-satchel.
- **Palette:** U palette (`#4a90d9` → `#16294f`, accent `#a8d4f7`) — bright sky-blues and white, gilt caduceus. Greek marble + gilt; Aegean-blue sky streaks.
- **Lighting:** Bright high sky key; cool wind-rim on the trailing edges.
- **Expression:** Sly grin, quick bright eyes cutting back at the viewer.
- **Props / weapon:** The gilt caduceus and winged sandals (canon kit); satchel.
- **Background:** Streaked open sky, motion-blurred cloud, one distant Olympian peak.
- **Holo interaction:** Sheen — the Shine sweep echoes her speed across sky-blue cloth; keep the open sky lightly textured with cloud so the sweep doesn't white it out.
- **Rarity ambition:** Uncommon — one figure plus speed-streak motion as story.
- **Prompt:** Genderbent Hermes dashing airborne across frame in sky-blue traveling chiton with winged petasos and winged sandals, thrusting a gilt caduceus forward, grinning back over her shoulder, streaked cloud sky with a distant peak, sly swift courier, bright sky key with cool wind rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Demeter, Harvest Mother — `gk-demeter`
- **Card facts:** {2}{G}{G} · G · 3/3 · r · holo: auto:sheen
- **Character & source:** Demeter as-is, goddess of the harvest; her ETB ramps a basic land — she makes the earth yield. A grounded, generous power with a hard edge.
- **Personality / mood:** "Cross her and see what grows in your fields: nothing." Nurturing to her own, ruinous to her foes.
- **Pose & composition:** Standing three-quarter, one arm sweeping a sheaf of wheat, the other scattering grain that sprouts as it falls (the ramp made visible); face ≈ y 320, warm and steady. Environmental story element (uncommon): a stalk of wheat springing up mid-air where her hand passes.
- **Costume & attire:** Golden-green harvest peplos, wheat-woven crown and belt, earth-toned mantle, gilt sickle at the hip.
- **Palette:** G palette (`#4fa06a` → `#123a22`, accent `#a9dcae`) — field-greens and ripe gold-wheat, warm loam. Greek marble + gilt sickle.
- **Lighting:** Warm low-sun harvest key gilding the wheat; cool sky rim behind.
- **Expression:** Serene, maternal, with a flicker of warning behind the eyes.
- **Props / weapon:** Wheat sheaf, scattering grain, a gilt harvest sickle.
- **Background:** A ripe golden field under a low sun, furrows receding.
- **Holo interaction:** Sheen — the Shine sweep runs across gold wheat and green cloth; keep the bright wheat field textured so the sweep reads as sun-sheen, not a white flat.
- **Rarity ambition:** Uncommon — one figure plus the sprouting-grain story of her ramp.
- **Prompt:** Demeter in a golden-green harvest peplos with wheat-woven crown, sweeping a sheaf of wheat and scattering sprouting grain, gilt sickle at hip, ripe golden field under a low sun, serene maternal harvest-goddess with a hard glint, warm harvest key with cool sky rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Hecate, Crossroads Witch — `gk-hecate`
- **Card facts:** {2}{B}{B} · B · 3/3 · r · holo: auto:sheen
- **Character & source:** Hecate as-is, triple goddess of witchcraft and crossroads; her ETB forces a random discard — she robs a foe's plans at the threshold.
- **Personality / mood:** "Three faces, three roads, and all of them toll roads." Enigmatic, transactional, everywhere three ways at once.
- **Pose & composition:** Standing three-quarter at a crossroads, twin torches raised, a subtle triple-face motif (two profiles ghosted behind the main face) reading as her three aspects; face ≈ y 315, gaze steady and appraising. Environmental element (uncommon): a stolen card-scrap dissolving into ash from a foe's hand.
- **Costume & attire:** Deep-violet-and-black robes, a keyed girdle, moon-crescent diadem, dark veil half-drawn.
- **Palette:** B palette (`#5a3a70` → `#140d1c`, accent `#9b6fc4`) — violet and black with cold torch-glow. Greek marble + gilt on the keys and torches.
- **Lighting:** Twin cold-violet torch-glow as key uplighting the three faces; a single warm gilt rim.
- **Expression:** Knowing, unreadable, faintly amused across all three aspects.
- **Props / weapon:** Twin torches, a ring of keys, dissolving ash of a stolen plan.
- **Background:** A three-way crossroads at night, a boundary-herm stone, dark cypress.
- **Holo interaction:** Sheen — the Shine sweep glides over violet robes and torch-lit trim; keep large black fields slightly textured so the sweep doesn't wash them.
- **Rarity ambition:** Uncommon — one figure plus the discard's dissolving-ash story.
- **Prompt:** Hecate at a night crossroads in violet-and-black robes with a moon-crescent diadem, raising twin torches, two ghosted profiles behind her main face for her three aspects, a stolen scrap dissolving to ash, boundary-herm and dark cypress, enigmatic triple witch, cold violet torch key with a gilt rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Nyx, Primordial Night — `gk-nyx`
- **Card facts:** {3}{B}{B} · B · 4/4 · flying · r · holo: auto:sheen
- **Character & source:** Nyx as-is, the primordial goddess of night; a 4/4 flyer — vast, dark, and quietly authoritative over all Olympus.
- **Personality / mood:** "Even Zeus lowers her voice after dark. It's only polite." Immense, calm, older than the gods and unbothered by them.
- **Pose & composition:** Aloft three-quarter, dark star-strewn wings and veil-mantle spread across the frame, one hand drawing the night sky closed like a curtain; face ≈ y 305 serene, gazing down. Great and drifting, not aggressive.
- **Costume & attire:** A flowing blue-black chiton and veil sewn with points of starlight, obsidian diadem, a crescent brooch, dark feathered wings.
- **Palette:** B palette (`#5a3a70` → `#140d1c`, accent `#9b6fc4`) — deep indigo-black with cold amethyst star-points. Greek marble + gilt, minimal; Aegean-blue night at the horizon.
- **Lighting:** Cool starlight key from within her mantle; a thin warm gilt rim tracing wing and jaw.
- **Expression:** Serene, sovereign, quietly all-seeing.
- **Props / weapon:** The starred veil-mantle drawn across the sky; no weapon.
- **Background:** The deep night sky itself, star-scattered, a dim horizon glow below.
- **Holo interaction:** Sheen — the Shine sweep glides over the star-flecked mantle; keep the dark sky textured with stars so the sweep reads as night-shimmer, not a wash.
- **Rarity ambition:** Uncommon — one figure with the night-mantle as its single grand idea.
- **Prompt:** Nyx aloft in a star-sewn blue-black chiton and veil-mantle with obsidian diadem and dark feathered wings, drawing the night sky closed like a curtain, deep star-scattered sky with a dim horizon glow, serene primordial night-goddess, cool starlight key with a thin gilt rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Selene, Moonlit Charioteer — `gk-selene`
- **Card facts:** {2}{U} · U · 2/3 · flying · c · holo: none
- **Character & source:** Selene as-is, driver of the moon; a 2/3 flyer — she rides her chariot across the night. One clear idea.
- **Personality / mood:** "Drives the moon across the sky nightly. Parking it is the hard part." Dreamy, dutiful, a little wry about the routine.
- **Pose & composition:** Chariot-borne three-quarter, reins in hand, moon-disc glowing at her back, face ≈ y 315 turned to the viewer; the composition centered on the moon behind her. Single idea — the moon's charioteer — reads at 119×86.
- **Costume & attire:** Silver-and-blue flowing chiton, a crescent-moon diadem, pale veil streaming, silver-trimmed reins.
- **Palette:** U palette (`#4a90d9` → `#16294f`, accent `#a8d4f7`) — moon-silver and night-blue. Greek marble + gilt kept as silver-gilt trim.
- **Lighting:** Cool moon-disc backlight as key (halo behind her); soft silver rim on veil and reins.
- **Expression:** Calm, half-dreaming, gentle.
- **Props / weapon:** Chariot reins, the glowing moon-disc; no weapon.
- **Background:** Two-value night sky, the moon-disc her single strong element.
- **Holo interaction:** none — common.
- **Rarity ambition:** Common — single figure, one idea (the moon's charioteer).
- **Prompt:** Selene chariot-borne in a silver-and-blue chiton with a crescent-moon diadem, reins in hand, a glowing moon-disc haloed at her back, calm night sky, dreamy lunar charioteer, cool moon backlight with soft silver rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Iris, Rainbow Herald — `gk-iris`
- **Card facts:** {1}{W} · W · 2/1 · flying · c · holo: none
- **Character & source:** Iris as-is, rainbow messenger of the gods; a 2/1 flyer — swift, bright, and mockingly cheerful. One clear idea.
- **Personality / mood:** "Message for you: you're losing. Lovely colors, though." Bright, blithe, delivering bad news with a smile.
- **Pose & composition:** Airborne descent, one wing spread, a rainbow arc trailing from her heels across the frame, face ≈ y 305 tilted at the viewer with a bright grin; the rainbow and wing-tip break the top band. Single idea — winged rainbow herald.
- **Costume & attire:** White chiton shot with iridescent rainbow sheen at the hems, gilt herald's fillet, small feathered wings, a message-scroll in hand.
- **Palette:** W palette (`#f2e8cf` → `#c9a84c`, accent `#fffef2`) — luminous white with a single rainbow-arc accent (the one place spectrum color appears). Greek marble + gilt.
- **Lighting:** Bright sky key from above; warm gilt rim on the fillet and scroll.
- **Expression:** Cheerful, teasing, bright-eyed.
- **Props / weapon:** The trailing rainbow arc and a message-scroll; no weapon.
- **Background:** Open bright sky, the rainbow her one strong element, kept simple.
- **Holo interaction:** none — common.
- **Rarity ambition:** Common — single figure, one idea (winged rainbow herald).
- **Prompt:** Iris in flight in a white chiton with iridescent rainbow-sheened hems and gilt herald's fillet, small wings spread, a rainbow arc trailing from her heels, message-scroll in hand, open bright sky, cheerful teasing rainbow-herald, bright sky key with warm gilt rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Gaia, the World-Mother — `gk-gaia`
- **Card facts:** {4}{G}{G} · G · 6/6 · trample · ur, legendary · holo: galaxy (explicit)
- **Character & source:** Gaia as-is, the primordial earth herself; her upkeep +1/+1 counter means she grows without end, a slow-waking mountain done being patient. Trample: nothing stops her.
- **Personality / mood:** "Older than the gods, patient as bedrock, done being quiet." Titanic, tectonic, finally rising.
- **Pose & composition:** Immense low-angle three-quarter rising from the land itself, shoulders and stone-crowned head towering, one hand cracking the earth open; face ≈ y 300 gazing down from a great height, the crown breaking the top band. Overwhelming mass, slow force.
- **Costume & attire:** A living body of moss-green cloth, root-woven mantle, stone-and-crystal crown, bark-and-vine armor across the shoulders, gilt-ore veins.
- **Palette:** G palette (`#4fa06a` → `#123a22`, accent `#a9dcae`) pushed deep — mineral greens, dark loam, gilt-ore glints. Greek marble + gilt; marble strata in the exposed rock.
- **Lighting:** Warm low earth-fire key from the cracking ground; cool dawn rim on the crown and shoulders.
- **Expression:** Ancient, unhurried, immovable — a mountain opening its eyes.
- **Props / weapon:** The cracking earth, stone crown, root-mantle; she is her own weapon.
- **Background:** A dark, high-contrast primordial landscape — riven rock, deep-shadow chasms, a star-broken dawn sky above.
- **Holo interaction:** Galaxy — the nebula SCREEN-blends into the dark chasms and shadowed rock, making a cosmos churn in the earth; keep the dark strata deep-valued, her face bright, and the crystal crown a clean specular edge for star glints.
- **Rarity ambition:** Gold-legendary primordial-titan splash — box-art tier.
- **Prompt:** Gaia rising titanic from the land in moss-green cloth and root-woven mantle with a stone-and-crystal crown, cracking the earth open with one hand, dark riven-rock landscape with deep chasms and a star-broken dawn sky, ancient immovable world-mother, warm earth-fire key with cool dawn rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Thanatos, Gentle End — `gk-thanatos`
- **Card facts:** {2}{B} · B · 2/2 · deathtouch · c · holo: none
- **Character & source:** Genderbent Thanatos, the gentle personification of death; deathtouch means her single quiet touch is always enough. One clear idea.
- **Personality / mood:** "Not cruel, never late, terribly good at goodbyes." Soft, sorrowful, kind — an end without malice.
- **Pose & composition:** Standing three-quarter, one hand lowered to extinguish a small guttering candle-flame (the deathtouch made gentle), face ≈ y 335 downturned, eyes soft and lidded; a single black moth on her wrist. One idea — the gentle end — reads at 119×86. No lunge.
- **Costume & attire:** Muted black-and-grey draped chiton, dark folded wings, an inverted-torch motif on the sash, a plain dull-silver circlet.
- **Palette:** B palette (`#5a3a70` → `#140d1c`, accent `#9b6fc4`) — soft charcoal and cold amethyst, one dim candle-warm note. Greek marble + gilt, faint and tarnished.
- **Lighting:** The dying candle-flame as low warm key; cool amethyst rim on the wings.
- **Expression:** Tender, sorrowful, at peace.
- **Props / weapon:** The guttering candle and inverted torch; a resting black moth — quiet, lethal.
- **Background:** Two-value dim threshold, a doorway fading to soft dark.
- **Holo interaction:** none — common.
- **Rarity ambition:** Common — single figure, one idea (the gentle end).
- **Prompt:** Genderbent Thanatos in muted black-and-grey draped chiton with dark folded wings, lowering one hand to extinguish a small guttering candle-flame, a black moth on her wrist, dim fading threshold, tender sorrowful death-goddess, low warm candle key with cool amethyst rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Pan, Wild Piper — `gk-pan`
- **Card facts:** {2}{G} · G · 3/3 · c · holo: none
- **Character & source:** Genderbent Pan, wild god of the pastures and panic; a plain 3/3 whose flavor is her fearsome flute. One clear idea. Kit-faithful: the syrinx pan-pipes.
- **Personality / mood:** "Her flute solo cleared three valleys. Critics called it 'panic.'" Feral, mischievous, unpredictable.
- **Pose & composition:** Standing three-quarter mid-pipe, syrinx to her lips, goat-legs braced on a mossy rock, face ≈ y 330 with a wild sidelong grin; horns break slightly into the top band. One idea — the wild piper — reads at 119×86.
- **Costume & attire:** Rustic green-and-earth wraps and a fur half-mantle, small ram's horns, ivy crown, goat-leg fauna traits, a hip-slung water-skin.
- **Palette:** G palette (`#4fa06a` → `#123a22`, accent `#a9dcae`) — pasture-greens and warm earth-browns. Greek marble + gilt kept minimal on the pipes.
- **Lighting:** Warm dappled forest-daylight key; cool leaf-shadow rim.
- **Expression:** Wild, grinning, eyes bright with mischief.
- **Props / weapon:** The syrinx pan-pipes (canon kit); ivy crown, horns.
- **Background:** Two-value sunlit glade, dappled leaf-light, no landmarks.
- **Holo interaction:** none — common.
- **Rarity ambition:** Common — single figure, one idea (the wild piper).
- **Prompt:** Genderbent Pan mid-pipe with a syrinx to her lips, small ram's horns and ivy crown, rustic green-and-earth wraps with a fur half-mantle and goat legs braced on a mossy rock, sunlit dappled glade, wild grinning piper, warm dappled daylight key with cool leaf-shadow rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait

### Eos, Rosy Dawn — `gk-eos`
- **Card facts:** {2}{W} · W · 2/2 · flying, vigilance · c · holo: none
- **Character & source:** Eos as-is, goddess of the dawn; a 2/2 flyer with vigilance — she rises each morning and never rests. One clear idea.
- **Personality / mood:** "Up before everyone, judging accordingly." Bright, early, faintly superior.
- **Pose & composition:** Airborne ascent three-quarter, arms opening the dawn, rose-gold wings spread, face ≈ y 310 lifted toward the coming sun with a knowing look; wing-tips break the band's sides. Vigilance reads as watchful, upward, ever-waking. One idea — the rosy dawn.
- **Costume & attire:** Rose-and-white flowing chiton with saffron sash, gilt sun-fillet, rose-gold feathered wings, dawn-dew jewelry.
- **Palette:** W palette (`#f2e8cf` → `#c9a84c`, accent `#fffef2`) warmed to rose-gold — pink-white cloth, saffron, gilt. Greek marble + gilt; Aegean-blue pre-dawn sky.
- **Lighting:** Warm rose-gold sunrise key from below the horizon; cool blue pre-dawn rim.
- **Expression:** Serene, knowing, faintly superior smile.
- **Props / weapon:** The opening dawn-light, saffron sash; no weapon.
- **Background:** Two-value pre-dawn sky flushing rose at the horizon, one strong element.
- **Holo interaction:** none — common.
- **Rarity ambition:** Common — single figure, one idea (the rosy dawn).
- **Prompt:** Eos ascending in a rose-and-white chiton with saffron sash and gilt sun-fillet, rose-gold wings spread, arms opening the dawn, pre-dawn sky flushing rose at the horizon, serene knowing dawn-goddess, warm sunrise key with cool blue pre-dawn rim — crisp cel-shaded gacha anime splash art, fully rendered scenic background, 640×800 portrait
