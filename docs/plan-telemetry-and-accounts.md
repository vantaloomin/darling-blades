...
| ~~`cardsPlayed`~~ | moved off this event | **Ruled 2026-09-17 (D-T0.1):** played cards are tallied in memory across one launch and sent as a third event, `cards`: one row per distinct `cardId` with a bucketed count, no duel reference, no deck reference, plus one bucketed count of the duels the launch contained, identical on every row: a denominator, not a reference (owner ruling 2026-09-17). **Amended 2026-09-25 (D11, 1.8.1):** a batch goes at every hide (the tab or window hidden, or the page closed), not only the first, and carries only the cards no earlier batch of that launch carried, so each card is still reported once per launch and the k = 10 row floor still counts launches. A card's count covers its plays from launch start up to the batch that carries it; the duel count is the launch's running total when the batch leaves (not a count since the last batch), so both cover the same span, and no rollup number adds the duel count up. The first 60 distinct cards per launch are reported, across all its batches. A hide with nothing new sends nothing. Nothing is stored on the device for it. See the k-anonymity rule below |
