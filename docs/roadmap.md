...
- **1st expansion: "Ragnarök" (69 collectible cards + 3 tokens; set `ragnarok`,
  prefix `rg-`).** A Norse headline faction (Valkyries, Norns, Jotun, Draugr, the
  ...
  **Ragnarök booster** SKU (set-scoped `packPool`/`openPack`; the shop shows two
  ...
  (Card-facts computed from the data → checker-clean); card art still renders
  procedural placeholders until a `gen-card-art --faction ragnarok` run. Added
