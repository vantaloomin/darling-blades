...
| **Duty** | Activated ability with {T} in its cost *(the "tap ability"; not a keyword in Magic, but the shape every set since Alpha has printed)* | "Tap this permanent, and pay any listed cost, during your Morning or Afternoon to perform its Duty. A permanent cannot tap the turn it arrives unless it has Warcry." | **SHIPPED (1.8, engine PR #355; AI and duel UI PR #359):** `CardDef.activated {cost:{tap, mana?}, ops, targets?}` + the stack-free main-phase `activate` action with inline targets (replay log v12; rules revision unchanged), `activatedBlockers`/`canActivate` legality, off-stack resolution followed by a state-based check, Foresee continuation under the activator's context, creature/artifact/enchantment carriers only (never lands, never beside Hauntlink or `manaAbility`), `activatedPolicy` AI at every difficulty (non-creatures use free abilities early, creatures only after combat). The card face shows the tap pip, never the word; the glossary teaches the name Duty and blades-db translates it to "activated ability". Tap itself stays out of scope above; this row is the ability shape, not the verb. |
| **Whispers** | Madness *(the cast-from-discard shape; timing differs)* | "Whispers {N}: while this card is fresh in your graveyard (it came from a hand or a deck, and your opponent's next Dawn has not come), you may cast it for {N}." | **SHIPPED (1.8, engine PR #367):** `CardDef.whispers {cost}` + `castSpell.whispers` on the existing action (replay log v13, rules revision unchanged); the `whispersUntilDawnOf` marker written only for hand and deck origins and cleared at that player's Dawn; Retell's graveyard source model; window rules of the card's own type; never beside Retell, Rite, Hauntlink or X, no Empower. AI and duel UI in the wave's second PR. blades-db translates it to "madness". |
| **Tithe** | Emerge *(approx.; the discount-by-sacrifice shape, any number of bodies)* | "Tithe: as an additional cost to cast this, you may sacrifice any number of creatures; each two Defense among them pays one generic mana." | **SHIPPED (1.8, engine PR #367):** `CardDef.tithe {per: 2}` + `castSpell.tithe` with the fodder in Rite's `sacrifices` field; `castCost` takes floor(Defense / 2) off the generic part only; canonical-set enumeration with permissive validation; Rite's payment order and creature-cap rule; never beside X, Retell, Hauntlink, Whispers or Rite. Horror-only is a per-set catalog test, not the validator. `tithePolicy` AI and the sacrifice picker in the wave's second PR. blades-db translates it to "emerge". |
