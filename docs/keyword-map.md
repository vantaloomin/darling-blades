...
| **Hauntlink** | Bestow *(approx.)* | "Hauntlink {cost}: You may play this linked to a creature you control. The linked creature gains [the Linked rider]. When it leaves play, put this into its owner's graveyard." | **Planned (1.5, Yokai Nights headline — confirmed 2026-07-28):** an alternate play mode on Artifacts/Enchantments (the Retell alternative-cost seam is precedent) — one host pointer, one leaves-play cleanup rule, no reattach/move; AI host-selection value at all difficulties. Spec: [plan-1.5.md](plan-1.5.md) Pillar 0 + the yokai-nights overplan draft. |
